...
const emptyVideo = { file: null, title: '', description: '' };
...
const CourseSectionForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const videoMeta = videos.map(({ title, description }) => ({ title, description }));
      formData.append('videos', JSON.stringify(videoMeta));
      // Append all video files
      videos.forEach((video) => {
        if (video.file) formData.append('videos', video.file);
      });
      await axios.post('http://localhost:5000/admin/course-section', formData, {
      ...
      alert('Error creating course section. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-3xl font-bold mb-6 text-center text-gray-800">Create Course Section</h2>
